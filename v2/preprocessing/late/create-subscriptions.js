...
function createSubscriptions(node) {
    ...
    if (node.type === 'element') {
        ...
        reactiveOriginDeps.map(rod => {
          ...
          const updateFunctionSnippet = UTIL.makeSnippet(G.SNIPPETS.CREATE_UPDATE_FUNCTION, {name: updateFunctionRedefine, elementName: node.varName, elementTag: node.tag, reactiveDependency: rod.name, reactiveProps: underlayingReactiveProps})
          ...
          const subscriptionSnippet = UTIL.makeSnippet(G.SNIPPETS.SUBSCRIBE, 
